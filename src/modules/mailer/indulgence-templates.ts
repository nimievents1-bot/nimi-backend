...
/**
 * Birthday — sent by daily cron at 09:00 to anyone whose DD/MM matches
 * today. The `promoCode` is a per-user, one-time, unguessable code
 * generated by `PromoCodesService.issueBirthdayCode`. We deliberately
 * tell the customer the code is "for you" rather than a generic
 * coupon, because sharing it with a friend won't work — the API
 * binds it to their user id and will reject a redemption by anyone
 * else.
 */
...
<body><div class="card">
  ...
  <p>Your personal code for the next ${validDays} day${validDays === 1 ? "" : "s"}:</p>
  <p><span class="code">${escape(promoCode)}</span></p>
  <p>Enter it at checkout on any pastry order and we'll take care of the rest. The code is tied to your account, so it's just for you.</p>
...
As part of The Nimi Indulgence Club, we've added a small treat to celebrate you — a moment of indulgence on us.

Your personal code (valid for ${validDays} day${validDays === 1 ? "" : "s"}):

  ${promoCode}

Enter it at checkout on any pastry order and we'll take care of the rest. The code is tied to your account, so it's just for you.
